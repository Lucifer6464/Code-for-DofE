...
best = 0
# creates a loop that runs the training process multiple times
for _ in range(100):
    x_train, x_test, y_train, y_test = sklearn.model_selection.train_test_split(X, y, test_size=0.1)
    ...
    print(acc)
    # applies the highest accuracy model and then saves that
    if acc > best:
        best = acc
        # save as pickle
...
# this section of code can be used to see correlation between
# points, G3 is on the y axis and p (which can be changed)
# is on the x axis so you can see how it correlates to G3
p = 'studytime'
style.use("ggplot")
